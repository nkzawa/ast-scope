...
Scope.prototype.undeclared = function(name) {
  if (typeof name !== 'string') {
    name = name.name;
  }

  var scope = this.ancestors().pop() || this;
  var variable = scope.undeclaredVariables[name];
  if (!variable) {
    variable = new Variable({type: 'Identifier', name: name});
    scope.undeclaredVariables[name] = variable;
  }
  return variable;
};
...
Scope.prototype.resolveVariable = function(name) {
  ...
  return scope.variables[name] || scope.undeclaredVariables[name];
};
